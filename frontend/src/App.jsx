...
import { askChat, changeStrategy, decidePricing, decideReorder, getPricingSuggestions, getProducts, getReorderSuggestions, getStrategy, simulateOrder, streamPricing } from './api'
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      setError('')
      const [productData, pricingData, reorderData] = await Promise.all([getProducts(), getPricingSuggestions(), getReorderSuggestions()])
  ...
  const spikes = products.filter(product => product.demandVelocity > 10)
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard"><header className="topbar"><div className="mobile-header"><button className="mobile-menu-button" aria-label="Open navigation" aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen(true)}><span /><span /><span /></button><div><p className="eyebrow">MONDAY, AUGUST 24, 2026</p><h1>{pageTitle(activeNav)}</h1><p className="subhead">{pageSubtitle(activeNav, pending)}</p></div><ThemeToggle theme={theme} setTheme={setTheme} /></div><div className="top-actions"><ThemeToggle theme={theme} setTheme={setTheme} /><button className="icon-button" aria-label="Notifications">&#128276;<span>{pending}</span></button><button className="profile">M <span>Merchandising</span> &#8964;</button></div></header>{error && <div className="error" role="alert">{error}</div>}
      ...
      {activeNav === 'Settings' && <SettingsPage strategy={strategy} loadStrategy={async () => { try { const result = await getStrategy(); setStrategy(result.strategy) } catch (cause) { setError(cause.message) } }} setStrategy={async value => { try { const result = await changeStrategy(value); setStrategy(result.strategy) } catch (cause) { setError(cause.message) } }} />}
    </main>
    <ChatWidget />
  </div>
}
