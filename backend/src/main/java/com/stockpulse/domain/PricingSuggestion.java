...
@Entity @Table(name = "pricing_suggestions", uniqueConstraints = @UniqueConstraint(name = "uk_pricing_pending_key", columnNames = "pending_key"))
public class PricingSuggestion {
    ...
    public PricingSuggestion(Product product, BigDecimal currentPrice, BigDecimal recommendedPrice, ChangeDirection direction, double confidence, String reasoning, TriggerReason triggerReason) {
        this.product = product; this.currentPrice = currentPrice; this.recommendedPrice = recommendedPrice; this.direction = direction; this.confidence = confidence; this.reasoning = reasoning; this.triggerReason = triggerReason; this.pendingKey = product.getId() + ":" + triggerReason;
    }
    public void accept() { requirePending(); status = SuggestionStatus.ACCEPTED; pendingKey = null; product.applyPrice(recommendedPrice); }
    public void reject() { requirePending(); status = SuggestionStatus.REJECTED; pendingKey = null; product.clearPriceReviewPending(); }
